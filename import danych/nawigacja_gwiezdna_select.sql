...
WHERE
    tob.aktywne = 1
    AND tob.budzet_roczny_PLN > 50000000;
...
WHERE
    tn.specjalizacja = 'Warp'
    AND tn.lata_doswiadczenia > 20;
...
WHERE
    ttn.czy_bezpieczna = 0
    AND ttn.dlugosc_ly > 100;
...
FROM
    ...
    tbl_statki_kosmiczne AS tsk
WHERE
    tm.id_statku = tsk.id_statku
    AND tm.typ = 'Transport';
...
FROM
    ...
    tbl_misje AS tm
WHERE
    ttn.id_misji = tm.id_misji
    AND ttn.trudnosc = 'Ekstremalna';
...
FROM
    ...
    tbl_misje AS tm
WHERE
    tln.id_misji = tm.id_misji
    AND tln.typ_zdarzenia = 'Alarm';
...
WHERE
    tp.id_gwiazdy = tg.id_gwiazdy
    AND tg.id_konstelacji = tk.id_konstelacji;
...
WHERE
    tob.id_planety = tp.id_planety
    AND tp.id_gwiazdy = tg.id_gwiazdy;
...
WHERE
    tn.id_statku = tsk.id_statku
    AND tin.id_statku = tsk.id_statku
    AND tin.producent = 'NavTech';
...
WHERE
    tm.id_nawigatora = tn.id_nawigatora
    AND tm.id_statku = tsk.id_statku
    AND tm.status = 'Zakończona';
...
WHERE
    tln.id_misji = tm.id_misji
    AND tm.id_nawigatora = tn.id_nawigatora
    AND tm.id_statku = tsk.id_statku
    AND tln.typ_zdarzenia = 'Wejście Warp';
...
WHERE
    tln.id_misji = tm.id_misji
    AND tm.id_docelowej_planety = tp.id_planety
    AND tln.typ_zdarzenia = 'Odkrycie';
...
WHERE
    tn.id_statku = tsk.id_statku
    AND tm.id_nawigatora = tn.id_nawigatora
    AND ttn.id_misji = tm.id_misji
    AND tn.stopien = 'Admirał';
...
WHERE
    ...
    AND tm.id_statku = tsk.id_statku
    AND tin.id_statku = tsk.id_statku
    AND tln.typ_zdarzenia = 'Wyjście Warp';
...
WHERE
    ...
    AND tm.id_statku = tsk.id_statku
    AND ttn.id_misji = tm.id_misji
    AND tn.stopien IN ('Starszy Nawigator', 'Kapitan Nawigator');
...
WHERE
    ...
    AND tm.id_statku = tsk.id_statku
    AND tin.id_statku = tsk.id_statku
    AND tln.id_misji = tm.id_misji
    AND tm.status = 'Przerwana';
...
WHERE
    tln.id_misji = tm.id_misji
    AND tm.id_nawigatora = tn.id_nawigatora
    AND tn.id_statku = tsk.id_statku
    AND ttn.id_misji = tm.id_misji
    AND tm.id_docelowej_planety = tp.id_planety;